...
void WeaselServerApp::SetupMenuHandlers() {
  ...
  m_server.AddMenuHandler(
      ID_WEASELTRAY_T9KEYS,
      std::bind(execute, dir / L"WeaselDeployer.exe",
                std::wstring(L"/t9keys")));
  // 重启算法服务: 无参数启动自身, _tWinMain 会先关闭旧实例再拉起新实例
  m_server.AddMenuHandler(
      ID_WEASELTRAY_RERUN_SERVICE,
      std::bind(execute, dir / L"WeaselServer.exe", std::wstring()));
